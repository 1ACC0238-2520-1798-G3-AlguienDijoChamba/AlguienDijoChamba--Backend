...
[ProducesResponseType(400)]
public async Task<IActionResult> CreateReputationFromJobReview(
    [FromBody] CreateJobReviewReputationRequest request,
    CancellationToken cancellationToken)
{
    if (request == null) 
        return BadRequest("Request body cannot be null.");
    
    Console.WriteLine($"🔵 Endpoint POST /reputation/job/review - JobId: {request.JobId}");

    var command = new CreateReputationFromJobCommand(
        request.JobId,
        request.Rating,
        request.Review
    );
    ...
    var dto = reputationEntity as ReputationDto;
    return CreatedAtAction(nameof(GetReputation), new { professionalId = dto?.ProfessionalId }, reputationEntity);
}
